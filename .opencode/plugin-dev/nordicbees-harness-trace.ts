...
export const MAX_ROTATE_BYTES = 100 * 1024 * 1024 // 100 MB
export const HEARTBEAT_INTERVAL_MS = 5 * 60_000 // 5 minutes
export const TOOL_START_MAP_MAX = 5000

/** SHA-256 of `input`, truncated to the first `len` hex characters. */
export function sha256Prefix(input: string, len: number = HASH_PREFIX_LEN): string {
 ...
 * its own.
 */
export type InjectionSignature = { id: string; kind: "substring" | "exact"; pattern: string }

export const INJECTION_SIGNATURES: readonly InjectionSignature[] = [
 ...
 */
export function matchInjection(text: string): { id: string; promptHash: string } | null {
...
export function shouldRotate(sizeBytes: number, maxBytes: number = MAX_ROTATE_BYTES): boolean {
  return sizeBytes > maxBytes
}

/** Base filename for a rotated-out trace file, given an ISO date (YYYY-MM-DD). */
export function rotatedFileName(isoDate: string, suffix: number = 0): string {
 ...
 */
export function eventToRecord(event: unknown, nowIso: string, mono: number): Record<string, unknown> | null {
...
export function buildMessageCreatedRecord(input: {
  sessionID: string
  agent?: string
  role: string
  partCount: number
...
export function buildInjectionRecord(input: {
  sessionID: string
  signatureId: string
  promptHash: string
...
export function buildToolExecuteBeforeRecord(input: {
  sessionID: string
  callID: string
  tool: string
  args: unknown
...
export function buildToolExecuteAfterRecord(input: {
  sessionID: string
  callID: string
  tool: string
...
export function buildHeartbeatRecord(sessionsTracked: number, nowIso: string, mono: number): Record<string, unknown> {
  return { ...base("heartbeat", nowIso, mono), sessionsTracked }
}

/** Builds a `recorder_error` record — never thrown, always logged and swallowed. */
export function buildRecorderErrorRecord(error: unknown, nowIso: string, mono: number): Record<string, unknown> {
...
export default NordicBeesHarnessTrace
