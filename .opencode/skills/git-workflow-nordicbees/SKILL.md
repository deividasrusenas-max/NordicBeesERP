...
## Every commit must follow this exact sequence — order matters
...
3. `git status` — confirm only the intended file(s) are modified.
4. `git add [exact file path(s)]` — never `-A` or `.`.
5. `git commit -m "..."` (message format below).
6. `git log --oneline -1` — confirm the commit just made actually contains
   this task's file (`git show --stat HEAD` if unsure). Do not proceed to
   step 7 without confirming this via a real command result.
7. ONLY NOW run `./bump-version.sh patch` (this modifies
   `AssemblyInfo.cs`/`.csproj`, commits, tags, and pushes automatically —
   do not bump the version manually by editing the file yourself).

**Never run `bump-version.sh` before the code commit.** `bump-version.sh`
does its own separate `git add`/`commit`/`tag`/`push` for only
`appsettings.json`/`NordicBeesERP.csproj`/itself — it never touches the
task's actual file. Running it first has previously caused a pushed
version tag with no corresponding code commit when something failed
between steps (real incident, `Orders/Detail.razor`, July 2026) — always
commit the real change first, verify it landed, only then bump.
