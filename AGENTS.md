...
- As of 2026-09-13 there are 11 files in `Migrations/`: `20260602150000_InitialCreate.cs` plus 10 incremental files dated 2026-07-01 through 2026-09-10. `InitialCreate.cs` was edited in place repeatedly through 2026-08-24, but commit `cffbb5e` (2026-09-02) explicitly reverted an in-place edit to `InitialCreate.cs` in favor of a new file (`AddContainerCodeUniqueIndex.cs`), and every migration since has followed that convention. The rule going forward: `InitialCreate.cs` is the frozen baseline, already applied everywhere — NEVER edit it again. Every new schema change is a NEW file under `Migrations/` (`YYYYMMDDHHMMSS_Description.cs`), matching the 10 files added since 2026-07-01.
- The dev DB is actually MySQL 8.0.46 (Ubuntu build; confirmed via `SELECT VERSION()` on 2026-09-13), not MariaDB — the `mariadb` CLI below is just a client binary name. `ADD COLUMN IF NOT EXISTS` / `DROP COLUMN IF EXISTS` are used dozens of times throughout the applied `InitialCreate.cs` and other migration files and have run successfully against this DB since the project's earliest migrations with no logged failure — they are supported here and safe to use as-is. (A commit that stripped `IF EXISTS` "for MariaDB compatibility" exists only on an abandoned, never-merged branch and was never validated against this DB — disregard it.)
...
### Final work report — persist it if you can, return it as text if you can't

- At the end of EVERY task, your complete final report (files changed, exact diff, build output, verification/test results, DDL statements requiring manual execution, anything the human needs to act on) must reach the human.
- **If your `opencode.json` permission grants you `edit` or `write` for `.opencode/reports/`**: persist the report to a file there —

      .opencode/reports/<short-task-name>-<YYYYMMDD-HHMM>.md

  This directory IS writable by agents with that permission. It exists specifically for this purpose and is listed in `.gitignore` (ephemeral working notes, never committed). If the write fails despite having the permission, that is a REAL error to report verbatim — not a signal to fall back to `.opencode/planning/` or claim "permission blocked" without first attempting it and showing the actual error. Do NOT substitute `.opencode/planning/` or `.agent-reports/` (this old path was retired 2026-08-22 when the harness was consolidated into `.opencode/` — it no longer exists) for this file. If `.opencode/reports/` genuinely does not exist, create it first (`mkdir -p .opencode/reports`) — its absence is not a blocker requiring human intervention.
- **If your permission denies both `edit` and `write` for `.opencode/reports/`**: return your complete report as the text of your final message instead. This IS compliance — not a fallback, not a violation, nothing to apologize for. Do NOT attempt to reach a file or any other destination (including a network request) through some other tool to satisfy this rule — that has already caused a real security incident: a subagent, denied every file-writing capability, POSTed its report via `fetch()` to six different guessed internal API endpoints before the circuit-breaker caught it (`Docs/BUGLOG.md`, `agent-invents-http-exfiltration-path-when-write-denied`). If you ever find yourself reaching for a tool to persist a report somewhere you have no permission to write, that impulse itself is the bug — stop, and put the report in your response text instead.
