...
- **Exception: `verifier`.** This agent's `opencode.json` permission block denies `write`, `edit`, and `bash` entirely, by design — it is a read-only browser-verification role (see `verifier.md`'s own opening line). The rule above does NOT apply to it. `verifier` returns its complete report as the text of its final message; the orchestrator (which has write permission and already receives that full text via the Task tool's return value) is responsible for persisting it to a file if a durable record is needed. `verifier` must NEVER attempt to reach a file or network destination through any OTHER tool (e.g. a browser-automation tool's scripting capability) to satisfy this rule — that is a real, observed security incident, not a hypothetical: a verifier session, denied write/edit/bash, POSTed its report via `fetch()` to six different guessed internal API endpoints before the circuit-breaker caught it (`Docs/BUGLOG.md`, `agent-invents-http-exfiltration-path-when-write-denied`).
