...
- **Guardrail added**: added to the `mudblazor` skill's known-pitfalls material (checkbox row-selection must use `T="bool"` + `Checked` bool-lookup + `CheckedChanged="val => Handler(id, val)"`, never a generic-`T` + `Value` combo for row selection) — soft prompt-text guardrail.
- **Category**: UI-form
...
- **Guardrail added**: none mechanical. This is a raw-SQL SELECT-expression vs `DbDataReader.GetInt*` cast-mismatch class. Candidate mechanical check: a `dotnet-efcore-nordicbees` note (soft) to read computed/SELECT-expression columns via the type the DB actually returns (e.g. a bare `<=0` boolean arrives as Int64, not Int32) — plus, ideally, a semgrep/analyzer rule flagging `GetInt32/GetInt64` calls fed from a non-literal SQL expression. Until such a rule exists, this class is guarded only by reviewer diligence + the Playwright E2E round-trip that surfaces it.
- **Category**: EF-core
...
- **Status**: monitoring — sibling class to `fixer-fabricated-commit-hash` (fabricated identifier vs. fabricated feature description); if a third fabrication variant appears, consider an umbrella escalation for "agent prose is not evidence" as a named guardrail category.
...
- **Guardrail added**: none mechanical — the four known sites are now annotated, but nothing prevents a future `decimal` property from shipping without `[Precision]`/`[Column(TypeName)]`. Candidate mechanical check: a semgrep/`agent-guardrails` rule flagging `decimal` EF model properties that lack a `[Precision(...)]` or `[Column(TypeName="decimal(...)")]` attribute (same escalation pattern as `ef-connection-pool-race-last-insert-id` — wire the existing config into pre-commit/CI so it cannot be skipped by an agent).
- **Category**: EF-core
- **Error class**: `ef-decimal-precision-annotation-missing`
- **Status**: N/A — no guardrail exists; the four known properties are fixed but the mechanism (decimal-without-precision → provider-default precision drift) remains unguarded for any future decimal property, and would silently resurface if `dotnet ef migrations add` is ever run against the live DB.
