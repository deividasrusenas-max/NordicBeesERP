...
- **Root cause**: `reviewer.md`'s Mode A requires a concluding verdict line, but nothing requires the response to contain the review's substance. The session satisfied the concluding-line requirement with a self-referential placeholder ("verdict delivered above") referencing content it never emitted, and the orchestrator-side check validates marker PRESENCE (APPROVED/REJECTED), not marker content. Related but distinct from `plan-without-execution-gap` (2026-09-06, reviewer Mode B): there the terminal verdict text was entirely ABSENT (a prompt gap, fixed by adding the mandatory Mode B concluding line); here the terminal marker is PRESENT but hollow — absent-marker vs present-but-hollow-marker are different mechanisms, so a new tag is minted rather than reusing that one.
- **Fix**: none in code (process/harness gap, not application code). For the T1 diff itself: the independent orchestrator verification described under Symptom. Going forward (T1.1 onward): every reviewer delegation explicitly demands a content-bearing verdict — literal APPROVED/REJECTED plus a short numbered checklist of the specific items checked, with line citations.
- **Guardrail added**: prompt-text only, tried live the same day and FAILED — the T1.1 Round B reviewer delegation carried an explicit MANDATORY OUTPUT FORMAT and the first hollow response quoted verbatim with an instruction not to repeat it; both attempts still returned one-line meta-summaries. A MECHANICAL check (e.g. a circuit-breaker/quality-monitor rule requiring the reviewer's final text part to exceed a minimum length or contain at least one specific evidence item) is now REQUIRED, not optional — prompt-text alone has a documented record of being unreliably weighted under context pressure.
- **Category**: infra (harness)
- **Error class**: `reviewer-verdict-without-content` (new tag)
- **Status**: N/A — no guardrail added for this entry; this error class is currently UNGUARDED, and periodic review should treat it as a top-priority candidate for a mechanical check.
