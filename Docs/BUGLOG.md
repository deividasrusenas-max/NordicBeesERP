...
- **Root cause (mechanical half, replayed against `.opencode/plugin/nordicbees-circuit-breaker.ts` as actually implemented, not reasoned about in the abstract)**: extracted `detectCyclicPattern()` verbatim and replayed the exact reported 4-call cycle (`A, B1, A, B2`) against it. Result: the pattern is well WITHIN the detector's own scope — period 4 is `<= CYCLE_MAX_PERIOD` (8), and `CYCLE_REPEAT_THRESHOLD` (5) is satisfied after exactly 5 repetitions of the true 4-step cycle, i.e. call #20 of the sequence (contrast: if it really had been the apparent period-2 shape, it would have fired even sooner, at call #10). The real incident ran to roughly 30 repetitions — 60+ calls — well past call #20, with zero corresponding `circuit-breaker.jsonl` entries. This means ESC did NOT simply beat the threshold (a legitimate, non-bug outcome that was explicitly checked for and ruled out here); mechanically, given the sequence as reported, the abort should already have fired multiple times over before the user intervened. Separately confirmed (not inferred from tool-name spelling alone) that MCP tool calls DO enter this same per-session `history` array in general: `circuit-breaker.jsonl`'s 2026-09-09 `agent-invents-http-exfiltration-path-when-write-denied` entry (`ses_f7a07b382ffe0Tr5Wy0Xk0dDPJ`) shows `identical-args-streak` — which reads the identical `state.history` that feeds `detectCyclicPattern` — firing correctly off six real `playwright_browser_run_code_unsafe` MCP calls, so "MCP calls aren't tracked at all" is ruled out as the explanation. The specific reason THIS session's history never tripped `detectCyclicPattern` despite being in-scope is NOT identified — no access to this session's raw event stream was available this session to inspect actual argsHash strings (e.g. JSON key-order variance between logically-identical calls would silently break the exact-string comparison this detector and `identical-args-streak` both rely on) or confirm the session was registered in `subagentSessions` at all. Left open deliberately — per instruction, the detector is not today's fix target.
...
- **Guardrail added**: prompt-text only (`coder.md`, above) — NOT a new mechanical check. Explicitly noting the mismatch this leaves: a MECHANICAL guardrail already exists for this exact behavioral family (`same-tool-streak`/`identical-args-streak`, added 2026-09-06; extended this same morning by `aa8c7f3`'s `cyclic-pattern-repeat` specifically to catch multi-step cycles like this one), and per the replay above this incident's shape was within that guardrail's stated scope, yet it did not visibly fire. Today's fix works upstream of that gap (stopping the wasteful retry before it accumulates enough calls to matter) rather than resolving why the mechanical layer stayed silent.
- **Category**: infra (harness)
- **Error class**: `self-diagnosed-loop-no-behavioral-stop` (recurrence — see 2026-08-28 below) for the behavioral half; also touches `circuit-breaker-blind-to-multi-step-cycles` (2026-09-10, below) for the mechanical-non-fire puzzle, though that entry's own original root cause (structural period-1-only blindness) is confirmed NOT the explanation here (this pattern's period-4 is within the fixed detector's scope per the replay) — flagged as a related but distinct open question under that tag, not a re-confirmation of its original mechanism.
- **Status**: escalated — `self-diagnosed-loop-no-behavioral-stop` already had a mechanical guardrail (2026-09-06, extended 2026-09-10) before this recurrence; the guardrail's real-world coverage of this exact shape remains unproven despite this being, per the replay, a case it should mechanically have caught.
