...
public class CustomerServiceTests : IClassFixture<DbTestFixture>
{
    ...
    [Fact]
    public async Task SaveCustomerAsync_Insert_PersistsVatVerificationFields()
    {
        var service = new CustomerService(_fixture.Factory, NullLogger<CustomerService>.Instance);

        var vatVerifiedAt = DateTime.UtcNow;
        var dto = new Customer
        {
            Id = 0,
            Name = $"Test VAT Insert Customer {Guid.NewGuid():N}",
            City = "Vilnius",
            CountryCode = "LT",
            Country = "Lithuania",
            DefaultLanguage = "lt",
        ...
        Assert.Equal(vatVerifiedAt, reloaded.VatVerifiedAt);
    ...
    [Fact]
    public async Task SaveCustomerAsync_Update_PersistsVatVerificationFields()
    {
        // Arrange: insert a base partner to get a real auto-incremented id
        await using var context = await _fixture.Factory.CreateDbContextAsync();

        var uniqueName = $"Test VAT Update Customer {Guid.NewGuid():N}";
        var now = DateTime.UtcNow;
